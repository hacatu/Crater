...
int main(){
	...
	fprintf(stderr, g == 2*p ? "\e[1;32m%s passed\e[0m\n" : "\e[1;31m%s failed\e[0m\n", "Scrambled/sorted contains checks");
	cr8r_vec_popl(&nums_asc, &cr8r_vecft_u64, &g);
	if(g){
		fprintf(stderr, "\e[1;31mPopped wrong value from beginning of sorted vector\e[0m\n");
	}
	cr8r_vec_popl(&nums_shuf, &cr8r_vecft_u64, &g);
	if(g){
		fprintf(stderr, "\e[1;31mPopped wrong value from beginning of scrambled vector\e[0m\n");
	}
	g = (uint64_t)cr8r_vec_foldl(&nums_shuf, &cr8r_vecft_u64, (void*)0, cr8r_default_acc_sum_u64);
	uint64_t sum = p*(p - 1)/2;
	if(g != sum){
		fprintf(stderr, "\e[1;31mWrong foldl linear checksum for scrambled vector\e[0m\n");
	}
	g = (uint64_t)cr8r_vec_foldr(&nums_shuf, &cr8r_vecft_u64, (void*)0, cr8r_default_acc_sum_u64);
	if(g != sum){
		fprintf(stderr, "\e[1;31mWrong foldr linear checksum for scrambled vector\e[0m\n");
	}
	...
	if(g != sum){
		fprintf(stderr, "\e[1;31mWrong foldr linear checksum for sorted vector\e[0m\n");
	}
	...
	if(acc[0] != sum2){
		fprintf(stderr, "\e[1;31mWrong foldr quadratic checksum for scrambled vector\e[0m\n");
	}
